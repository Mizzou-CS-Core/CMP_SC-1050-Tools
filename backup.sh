...
while read p; do 
    if ! cp -rL /group/cs1050/submissions/1050/A/"$1"/"$p" "$new_directory" 2> /dev/null ; then
...
if test -f "$missing"; then
    while read p || [[ -n $p ]]; do 
    if ! cp -rL /group/cs1050/submissions/1050/A/"$1"/.invalid/*"$p"* "$new_directory" 2> /dev/null ; then
